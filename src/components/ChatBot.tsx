...
const ChatBot = () => {
  ...
  const quickActions = ["Tell me about Science stream", "Show me scholarships", "Engineering careers", "Best colleges near me", "Help with course selection"];
  if (!isOpen) {
    return <Button onClick={() => setIsOpen(true)} variant="hero" className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg z-40 text-lg">
        <MessageCircle className="h-6 w-6" />
      </Button>;
  }
  return <Card className={`fixed bottom-6 right-6 z-50 transition-all duration-300 ${isMinimized ? 'w-80 h-16' : 'w-80 h-96'} shadow-2xl`}>
      ...
      {!isMinimized && <CardContent className="p-0 flex flex-col h-80">
          <ScrollArea className="flex-1 p-4">
            <div className="space-y-4">
              {messages.map(message => <div key={message.id} className={`flex gap-2 ${message.isBot ? 'justify-start' : 'justify-end'}`}>
                  {message.isBot && <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center">
                      <Bot className="h-4 w-4 text-white" />
                    </div>}
                  <div className={`max-w-[70%] p-3 rounded-lg text-sm ${message.isBot ? 'bg-muted text-foreground' : 'bg-primary text-primary-foreground'}`}>
                  ...
                  {!message.isBot && <div className="w-8 h-8 bg-secondary rounded-full flex items-center justify-center">
                      <User className="h-4 w-4 text-white" />
                    </div>}
                </div>)}
              
              {isTyping && <div className="flex gap-2 justify-start">
              ...
              {messages.length === 1 && <div className="space-y-2">
                  ...
                  {quickActions.map((action, index) => <button key={index} onClick={() => setInputValue(action)} className="w-full text-left p-2 text-xs bg-accent hover:bg-accent/80 rounded transition-colors">
                      {action}
                    </button>)}
                </div>}
            </div>
          ...
          <div className="p-4 border-t">
            <div className="flex gap-2">
              <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ask me about courses, colleges, scholarships..." className="flex-1" disabled={isTyping} />
              <Button onClick={handleSendMessage} disabled={!inputValue.trim() || isTyping} size="icon" variant="hero">
